...
import com.dicoding.sub1_appsstory.Database.RemoteKeys
...
class StoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        try {
            val response = apiService.getStrories(token, page, state.config.pageSize)

            val endOfPaginationReached = response.listStory.isEmpty()

            database.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    database.remoteKeysDao().deleteRemoteKeysId()
                    database.storyyDaoo().deleteAlls()
                }
                ...
                database.remoteKeysDao().InsertAlls(keys)
                database.storyyDaoo().InsertStory(response.listStory)
            }
